...
Function Set-DatabricksEnvironment {
	<#
		...
		A switch that can be used to derive API Root URL and Access Token from the environmetn variables used by Databricks CLI
		.PARAMETER UsingAzContext
		A switch that can be used to connect to Databricks using a token generated by the Az module
		.EXAMPLE
	...
	(
		...
		[Parameter(ParameterSetName = "AADAuthenticationAzContext", Mandatory = $true, Position = 3)]
		...
		[Parameter(ParameterSetName = "AADAuthenticationAzContext", Mandatory = $false, Position = 8)]
		...
		[Parameter(ParameterSetName = "AADAuthenticationAzContext", Mandatory = $false, Position = 9)]
	...
	process {
		...
		elseif ($PSCmdlet.ParameterSetName -ilike "AADAuthenticationResource*" -or $PSCmdlet.ParameterSetName -eq "AADAuthenticationAzureDevOpsServiceConnection") {
			...
			{
				# When using Azure DevOps Service Connection, we assume that the Az module is installed and we can use Get-AzResource to derive the URL
				$azResource = Get-AzResource -ResourceId $AzureResourceId
		...
		if ($PSCmdlet.ParameterSetName -ilike "AADAuthentication*") {
			...
			Write-Verbose "'Setting CloudProvider to 'Azure' ..."
			$script:dbCloudProvider = "Azure"
			...
			elseif ($UsingAzContext) {
				Write-Verbose "Using Az Context authentication ..."
				$azResource = Get-AzResource -ResourceId $AzureResourceId
		...
		Write-Verbose "Databricks Environment initialized!"
	}
}
